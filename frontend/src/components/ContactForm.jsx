    ...
    <form className="contact-form" onSubmit={handleSubmit}>
      ...
      <div className="form-grid form-grid--two">
        <label className="field">
          ...
          <input required value={form.firstName} onChange={(event) => setField('firstName', event.target.value)} placeholder="e.g. Maya" />
        </label>
        <label className="field">
          <span>Last name</span>
          <input value={form.lastName} onChange={(event) => setField('lastName', event.target.value)} placeholder="e.g. Chen" />
        </label>
        <label className="field">
          <span>Job title</span>
          <input value={form.title} onChange={(event) => setField('title', event.target.value)} placeholder="e.g. Product Director" />
        </label>
        <label className="field">
          <span>Company</span>
          <input value={form.company} onChange={(event) => setField('company', event.target.value)} placeholder="e.g. Northstar Studio" />
        </label>
      </div>

      <div className="form-section">
        <div className="form-section-heading">
          <span><Mail size={16} /> Email addresses</span>
          <button className="text-button" type="button" onClick={() => addItem('emails')}><Plus size={14} /> Add another</button>
        </div>
      ...
      <div className="form-section">
        <div className="form-section-heading">
          <span><Phone size={16} /> Phone numbers</span>
          <button className="text-button" type="button" onClick={() => addItem('phones')}><Plus size={14} /> Add another</button>
        </div>
